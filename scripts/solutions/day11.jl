...
function formatinput(input)
    ...
    monkeys = []
    for kmonkeyind ∈ monkeysind
        monkey = Dict("items" => [parse(Int, m.match) for m ∈ eachmatch(r"(\d+)", input[kmonkeyind+1])], # items
            ...
            "throws" => [parse(Int, match(r"(\d+)", input[k]).match) + 1 for k ∈ kmonkeyind .+ (5:-1:4)]) # reverse order
        push!(monkeys, monkey)
    end
    monkeys
end
...
function solution(monkeys; nround=20, divide=3)
    ...
    modulus = lcm([monkey["divisor"] for monkey ∈ monkeys])
    # Loop through rounds, Loop through monkeys
    for _ = 1:nround, (kmonkey, monkey) ∈ enumerate(monkeys_)
